...
ockam_error_t run_test_server(test_cli_params_t* p_params)
{
  ...
  while (true) {
    ...
    if (0 == strncmp(ENDING_LINE, (char*) receive_buffer, strlen(ENDING_LINE))) {
      ockam_log_info("Server loop found ending line");
      break;
    }
    else {
  ...
  ockam_log_info("Server receive finished");
  ...
  while (true) {
    ...
    if (++i == 100) {
      i = 0;
      ockam_log_info("Server send sleep start");
      usleep(100*1000);
      ockam_log_info("Server send sleep finish");
    }
  ...
  ockam_log_info("Server file compare finished");
