...
ockam_error_t xx_test_responder_prologue(key_establishment_xx* xx)
{
  ...
  if (ockam_error_has_error(&error)) {
    ockam_log_error("xx_test_responder_prologue: %s: %d", error.domain, error.code);
    goto exit;
  }
...
ockam_error_t test_responder_handshake(ockam_key_t* p_key)
{
  ...
  if (ockam_error_has_error(&error)) {
    ockam_log_error("Failed responder_epilogue: %s: %d", error.domain, error.code);
    goto exit;
  }
                                             ...
                                             ockam_writer_t**    pp_writer)
{
  ...
  if (ockam_error_has_error(&error)) {
    ockam_log_error("establish_responder_connection: %s: %d", error.domain, error.code);
    goto exit;
  }
...
ockam_error_t xx_test_responder(ockam_vault_t* p_vault, ockam_memory_t* p_memory, ockam_ip_address_t* ip_address)
{
  ...
  if (ockam_error_has_error(&error)) {
    ockam_log_error("ockam_responder_handshake: %s: %d", error.domain, error.code);
    goto exit;
  }
  ...
  if (scripted_xx) {
    ...
    if (ockam_error_has_error(&error)) {
      ockam_log_error("responder_epilogue_make failed: %s: %d", error.domain, error.code);
      goto exit;
    }
  }
